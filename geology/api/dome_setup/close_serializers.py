...
from geology.models import domeStatusClose,OreProductions
...
class DomeStatusCloseSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        qs = domeStatusClose.objects.filter(cek_duplicated=cek_dup)
        if self.instance:
            qs = qs.exclude(pk=self.instance.pk)

        if qs.exists():
            raise serializers.ValidationError({
                "non_field_errors": ["Data dome close dengan status ini sudah ada."]
            })
