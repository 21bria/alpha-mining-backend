...
class SamplesImporter:
    def run(self, rows: list[dict[str, Any]], user=None) -> ImportResult:
        ...
        seen_kode_batch_pds: set[tuple[Any, str]] = set()
        ...
        existing_pds_batch_keys: set[tuple[Any, str]] = set()
        existing_codes: set[str] = set()

        if db_iup_ids_needed:
            # existing iup + sample_number
            existing_rows = (
            ...
            # existing PDS kode_batch
            existing_pds_rows = (
                SampleProductions.objects
                .annotate(type_l=Lower("type"), kode_batch_l=Lower("kode_batch"))
                .filter(iup_id__in=db_iup_ids_needed, type_l="pds")
                .exclude(kode_batch__isnull=True)
                .exclude(kode_batch="")
                .values_list("iup_id", "kode_batch_l")
            )
            existing_pds_batch_keys = {
                (iup_id, kode_batch_l)
                for iup_id, kode_batch_l in existing_pds_rows
            }
        ...
        for item in parsed:
            ...
            try:
                ...
                # cek duplicate iup + sample_number
                ...
                if sample_type_final == "PDS":
                    ...
                    # sample_type_final = "PDS"

                    if kode_batch:
                        pds_key = (iup_id, kode_batch.casefold())

                        if pds_key in existing_pds_batch_keys:
                            raise ValueError(
                                f"duplicate PDS kode_batch in DB: iup_code '{item['iup_code']}', "
                                f"kode_batch '{kode_batch}' already exists"
                            )

                        if pds_key in seen_kode_batch_pds:
                            raise ValueError(
                                f"duplicate PDS kode_batch in file: iup_code '{item['iup_code']}', "
                                f"kode_batch '{kode_batch}'"
                            )

                        seen_kode_batch_pds.add(pds_key)
                ...
                # tambahkan ke existing set agar tidak duplicate dalam batch import yang sama
                existing_sample_keys.add(db_key)
                existing_codes.add(code)

                if kode_batch and sample_type_final == "PDS":
                    existing_pds_batch_keys.add((iup_id, kode_batch.casefold()))
