...
# @login_required
def get_data_crm_mral_plot_json(request):
    ...
    query = """
        SELECT
            ...
            sample_id,
            release_date,
            mral_ni, mral_fe, mral_mgo, mral_sio2
        FROM view_sample_crm_diff_mral
    ...
    for key in ["ni", "fe", "mgo", "sio2"]:
        df[f"cek_{key}"] = (
            (df[f"mral_{key}"] < df[key] + (df[key] * -0.1)) |
            (df[f"mral_{key}"] > df[key] + (df[key] *  0.1))
        ).astype(int)
