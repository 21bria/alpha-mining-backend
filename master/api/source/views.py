...
from .serializers import SourceMinesSerializer
from master.api.pagination import StandardResultsSetPagination
from master.api.base import MasterBaseViewSet

class SourceMinesViewSet(MasterBaseViewSet):
    queryset = SourceMines.objects.select_related("iup").all().order_by("sources_area")
    serializer_class = SourceMinesSerializer
    permission_classes = [
        IsAuthenticated,
    ...
    search_fields    = ["sources_area", "iup__iup_code", "iup__iup_name"]

    ordering_fields  = ["id", "sources_area",  "latitude", "longitude"]
    ...
    def download_template(self, request):
        file_path = os.path.join(
            ...
            "master",
            "import_templates",
            "SourceMines_import_template.xlsx"
        )

        return FileResponse(
            open(file_path, "rb"),
            as_attachment=True,
            filename="SourceMines_import_template.xlsx"
        )