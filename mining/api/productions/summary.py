...
class CompareSummaryView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        prod_qs = self.apply_iup_scope(mineProductionsView.objects.all(), request)
        prod_qs = apply_common_filters(prod_qs, request.query_params)

        total_all = sum_tonnage(prod_qs)

        total_mining = sum_tonnage(
            prod_qs.filter(category_mine__iexact="Mining")
        )

        total_project = sum_tonnage(
            prod_qs.filter(category_mine__iexact="Project")
        )

        return Response({
            "production_tonnage" : total_all,
