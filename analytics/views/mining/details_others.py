...
def get_detail_monthly(filter_year, filter_month, iup_filter=None):
    ...
    query = f"""
        ...
        actual AS (
            SELECT 
                ...
                SUM(
                    CASE 
                        WHEN nama_material IN ('Ballast', 'Biomass')
                        THEN tonnage ELSE 0 
                    END
    ...
    params = [
        tgl_pertama, tgl_terakhir,  # untuk generate_series
    ...
    df['achievement'] = df.apply(
        lambda r: round((r['total_tonnage'] / r['total_plan'] * 100), 2)
        if r['total_plan'] > 0 else 0.0,
        axis=1
    )
...
def get_detail_all(iup_filter=None):
    ...
    query = f""" 
        ...
        SELECT 
            ...
            COALESCE(a.total_tonnage, 0) AS total,
            COALESCE(p.total_plan, 0) AS plan_data
