...
from selling.models import DomeTransfer
from core.permissions import user_allowed_iup_ids
from selling.services.dome_transfer import (
    get_tonnage_by_dome,
    transfer_dome_selling,
    build_transfer_ref,
)


class DomeTransferSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        # Proteksi produksi sebelum transfer
        # if OreProductions.objects.filter(id_pile=original_dome.id).exists():
        #     raise serializers.ValidationError({
        #         "original_dome": (
        #             "Dome masih memiliki data produksi. "
        #             "Selesaikan/merge produksi dome terlebih dahulu sebelum transfer selling."
        #         )
        #     })
        ...
        if original_total <= Decimal("0"):
            raise serializers.ValidationError({
                "original_dome": "Original dome belum punya selling."
            })

        target_total = get_tonnage_by_dome(dome_second.id)
        if target_total <= Decimal("0"):
            raise serializers.ValidationError({
                "dome_second": "Target dome belum punya selling, tidak bisa transfer."
            })
    ...
    def create(self, validated_data):
        ...
        validated_data["ref_id"] = build_transfer_ref(original_dome.id, dome_second.id)
        validated_data["status"] = "TRANSFERRED"
        validated_data["is_undone"] = False

        merge_result = transfer_dome_selling(
    ...
    def update(self, instance, validated_data):
        raise serializers.ValidationError({
            "non_field_errors": ["Data transfer tidak boleh diubah. Gunakan undo jika ingin membatalkan."]
        })