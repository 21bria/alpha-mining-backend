...
def _apply_filters(qs, params):
    # filter selling
    qs = qs.filter(category__iexact="selling")
...
@register_exporter("selling.sample")
def export_selling_sample(job):
    qs = SamplesView.objects.all()
    qs = _apply_filters(qs, job.params or {})

    wb = Workbook()
    ws = wb.active
    ws.title = "Selling Samples"
    ...
    for row in qs.iterator(chunk_size=2000):
        ws.append([
            ...
            # row.area_sampling or "",
            # row.factory_stock or "",
            # row.point_sampling or "",
            row.selling_code or "",
            row.batch or "",
            row.increments or 0,
            row.size or "",
    ...
    filename = f"selling_sample_export_{job.id}.xlsx"
