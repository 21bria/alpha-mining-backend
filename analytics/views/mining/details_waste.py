...
def get_detail_monthly(filter_year, filter_month,iup_filter=None):
    ...
    actual AS (
        SELECT 
            ...
            SUM(CASE WHEN nama_material = 'Waste' THEN tonnage ELSE 0 END)::numeric AS total_tonnage
    ...
    plan AS (
        SELECT
            ...
            SUM(COALESCE(waste,0))::numeric AS total_plan
        FROM mining_plan_productions
        WHERE {where_plan}
...
def get_detail_weekly(filter_week,iup_filter=None):
    ...
    columns = ['tanggal', 'hari', 'waste', 'waste_plan', 'waste_ach']
    df = pd.DataFrame(data, columns=columns)

    # konversi numeric
    for col in ['waste', 'waste_plan']:
        df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0).astype(float)

    # total & achievement per hari
    df['total_actual'] = df['waste']
    df['total_plan']   = df['waste_plan']
    df['achievement']  = df.apply(
...
def get_detail_range(date_start, date_end,iup_filter=None):
    ...
    columns = ['tanggal', 'waste', 'waste_plan', 'waste_ach']
    df = pd.DataFrame(data, columns=columns)
...
def get_detail_yearly(yearly,iup_filter=None):
    ...
    columns = ['bulan', 'waste', 'waste_plan', 'waste_ach']
    df = pd.DataFrame(data, columns=columns)
...
def get_detail_all(iup_filter=None):
    ...
    query = f""" 
        ...
        SELECT 
            ...
            COALESCE(a.waste, 0) AS total,
            COALESCE(p.waste_plan, 0) AS plan_data
