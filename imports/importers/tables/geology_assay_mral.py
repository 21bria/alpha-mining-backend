...
def build_mral_code(iup_code: str, release_date, sample_id: str) -> str:
    ...
    return f"MRAL-{iup_val}-{d}-{sample_val}"
    ...
    def run(self, rows: list[dict[str, Any]], user=None) -> ImportResult:
        ...
        for row_no, row in enumerate(rows, start=1):
            try:
                ...
                if release_date and release_time:
                    release_mral = datetime.combine(release_date, release_time)
        ...
        if to_create:
            with transaction.atomic():
                AssayMral.objects.bulk_create(to_create, batch_size=500)
