-- view_geology_ore_production source
...
SELECT
    ...
    TRIM(BOTH FROM t11.dome) AS pit_dome,
    ...
    t1.direct,
    t1.sample_type AS sample_type_id,
    TRIM(BOTH FROM t8.type_sample) AS sample_type,
    t1.kode_batch,
    t1.iup_id,
    t9.iup_code,
    t9.iup_name,
...
LEFT JOIN master_mine_sources_pit_dome t11
  ON t11.id = t1.id_pit_dome
 AND t11.id_loading = t1.id_prospect_area
...
LEFT JOIN master_sample_type t8
  ON t8.id = t1.sample_type
